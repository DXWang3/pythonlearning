...
def find_centroid(restaurants):
    ...
    return [mean([restaurant_location(x)[0] for x in restaurants]), mean([restaurant_location(x)[1] for x in restaurants])]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xy = sum([(xs[i] - mean(xs)) * (ys[i] - mean(ys)) for i in range(len(ys))])
...
def rate_all(user, restaurants, feature_functions):
    ...
    return {restaurant_name(r) : user_rating(user, restaurant_name(r)) if r in reviewed else predictor(r) for r in restaurants.values()}
